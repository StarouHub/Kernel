...
try {
    ...
    // 11. DÉTECTION DE DUPLICATAS (NOUVEAU)
    $duplicate_detection = detectDuplicates($titre, $description);
    
    // 12. ANALYSE DE SIMILARITÉ (NOUVEAU)
    $similarity_analysis = analyzeSimilarity($titre, $description);
 ...
 * 11. DÉTECTION DE DUPLICATAS (NOUVEAU)
 */
function detectDuplicates($titre, $description) {
    try {
        ...
        if (empty($keywords)) {
            return ['has_duplicates' => false, 'similar_count' => 0];
        }
        
        // Rechercher des réclamations similaires
        $similarCount = 0;
        $similarReclamations = [];
        
        foreach ($keywords as $keyword) {
            $sql = "SELECT id, titre, description FROM reclamations 
                    WHERE (LOWER(titre) LIKE ? OR LOWER(description) LIKE ?)
                    AND statut != 'fermee'
                    LIMIT 5";
            $stmt = $db->prepare($sql);
            $searchTerm = '%' . $keyword . '%';
            ...
            if (count($results) > 0) {
                $similarCount += count($results);
                $similarReclamations = array_merge($similarReclamations, $results);
            }
        }
        
        // Dédupliquer
        $uniqueSimilar = [];
        foreach ($similarReclamations as $rec) {
            $uniqueSimilar[$rec['id']] = $rec;
        }
        
        return [
            'has_duplicates' => count($uniqueSimilar) > 0,
            'similar_count' => count($uniqueSimilar),
            'similar_reclamations' => array_values($uniqueSimilar)
        ];
    } catch (Exception $e) {
        return ['has_duplicates' => false, 'similar_count' => 0, 'error' => $e->getMessage()];
    }
}

/**
 * 12. ANALYSE DE SIMILARITÉ AVANCÉE (NOUVEAU)
 */
function analyzeSimilarity($titre, $description) {
    $duplicates = detectDuplicates($titre, $description);
    
    $similarityScore = 0;
    if ($duplicates['similar_count'] > 0) {
        $similarityScore = min(($duplicates['similar_count'] / 10) * 100, 100);
    }
    
    return [
        'similarity_score' => $similarityScore,
        'duplicate_detection' => $duplicates,
        'recommendation' => $duplicates['has_duplicates'] ? 
            "⚠️ Des réclamations similaires existent. Vérifiez si votre problème n'a pas déjà été signalé." : 
            "✅ Aucune réclamation similaire trouvée."
    ];
}
