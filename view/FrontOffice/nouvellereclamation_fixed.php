...
<body>
            ...
            <div>
                <div class="ai-panel">
                    ...
                    <div class="ai-content">
                        ...
                        <div class="ai-section">
                            <div class="ai-label"><i class="bi bi-graph-up"></i> Analyse</div>
                            <div id="aiAnalysis">
                                <small class="text-muted">Commencez à taper...</small>
                            </div>
                        </div>

                        <!-- SUGGESTIONS -->
                        <div class="ai-section">
                            <div class="ai-label"><i class="bi bi-star"></i> Suggestions</div>
                            <div id="suggestions">
                                <small class="text-muted">L'IA génèrera des suggestions...</small>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        // ===== AI INTELLIGENTE RÉELLE =====
        class RealAI {
            constructor() {
                this.keywordDatabase = {
                    critical: ['urgent', 'bloqué', 'crash', 'panne', 'grave', 'critique', 'données perdues', 'sécurité', 'hack'],
                    high: ['bug', 'erreur', 'problème', 'ne marche pas', 'ne fonctionne pas', 'impossible', 'exception'],
                    medium: ['lent', 'amélioration', 'suggestion', 'feature'],
                    low: ['typo', 'orthographe', 'style']
                };
            }

            analyze(titre, desc) {
                const text = (titre + ' ' + desc).toLowerCase();
                const score = this.calculateScore(titre, desc);
                const quality = this.assessQuality(score);
                const keywords = this.extractKeywords(text);
                const suggestions = this.generateSuggestions(titre, desc);
            ...
            extractKeywords(text) {
                const keywords = [];
                Object.values(this.keywordDatabase).flat().forEach(kw => {
            ...
            generateSuggestions(titre, desc) {
                ...
                if (titre.length < 15) {
                    suggestions.push('📝 Titre trop court - soyez plus descriptif');
                }
                if (desc.length < 100) {
                    suggestions.push('📝 Description insuffisante - développez davantage');
                }
                if (!desc.includes('\n')) {
                    suggestions.push('📋 Utilisez des sauts de ligne pour structurer');
                }
        ...
        function updateAI() {
            const result = ai.analyze(titre.value, description.value);
            ...
            let analysis = `<div style="background: #f8fafc; padding: 1rem; border-radius: 8px; text-align: center;">
                <strong>${result.quality}</strong><br>
            ...
            result.suggestions.forEach(sug => {
                suggestions += `<div style="background: #fffbeb; padding: 0.8rem; border-radius: 8px; margin-bottom: 0.5rem; font-size: 0.9rem; border-left: 3px solid #fcd34d;">
                    ${sug}
                </div>`;
            });
            suggestions += '</div>';
            document.getElementById('suggestions').innerHTML = suggestions || '<small class="text-success">✅ Tout va bien!</small>';
        }

        titre.addEventListener('input', updateAI);
        description.addEventListener('input', updateAI);
    </script>
</body>
</html>
