...
class ReclamationController {
    ...
    /**
     * NOUVEAU: Met à jour la priorité d'une réclamation (escalade manuelle)
     */
    public function updatePriority($id, $newPriority) {
        try {
            $allowedPriorities = ['basse', 'normale', 'haute', 'urgente', 'critique'];
            if (!in_array($newPriority, $allowedPriorities)) {
            ...
            $oldPriority = $oldData['priorite'];
            
            // Mettre à jour la priorité
            $updateSql = "UPDATE reclamations 
                         SET priorite = ?, 
                             priority_reason = CONCAT(COALESCE(priority_reason, ''), ' | Escalade manuelle: ', ?, ' → ', ?),
                             date_modification = NOW() 
                         WHERE id = ?";
            $updateStmt = $this->db->prepare($updateSql);
            $updateStmt->execute([$newPriority, $oldPriority, $newPriority, $id]);
            
            // Log de l'escalade manuelle
            $this->logActivity('manual_priority_escalation', [
                'reclamation_id' => $id,
            ...
            // Si escalade vers critique, notifier
            if ($newPriority === 'critique' && $oldPriority !== 'critique') {
                // Récupérer la réclamation pour l'escalade
                $reclamationData = $this->getDetails($id);
                if ($reclamationData['success'] && isset($reclamationData['reclamation'])) {
                    $r = new Reclamation();
                    $r->setTitre($reclamationData['reclamation']['titre']);
                    $r->setDescription($reclamationData['reclamation']['description']);
                    $r->setPriorite($newPriority);
                    $this->escalateCriticalPriority($id, $r);
                }
            }
            
            return [
                'success' => true, 
                'message' => "Priorité mise à jour de '$oldPriority' à '$newPriority'",
                'old_priority' => $oldPriority,
                'new_priority' => $newPriority
            ];
            
        } catch (Exception $e) {
            error_log("Erreur updatePriority: " . $e->getMessage());
            return ['success' => false, 'message' => 'Erreur lors de la mise à jour de la priorité'];
        }
    }
