...
class Sujet {
    ...
    public function getAll() {
        $stmt = $this->db->query("
            SELECT s.*, c.name as categorie_name,
                   (SELECT COUNT(*) FROM reponse WHERE sujet_id = s.id) as reponse_count
            FROM sujet s 
    ...
    public function create($titre, $contenu, $categorie_id, $user_id = null) {
        ...
        $stmt = $this->db->prepare("INSERT INTO sujet (titre, contenu, categorie_id, date_creation) VALUES (?, ?, ?, NOW())");
    ...
    public function update($id, $titre, $contenu, $categorie_id) {
        $stmt = $this->db->prepare("UPDATE sujet SET titre = ?, contenu = ?, categorie_id = ? WHERE id = ?");
        return $stmt->execute([$titre, $contenu, $categorie_id, $id]);
    }
    
    public function delete($id) {
        $stmt = $this->db->prepare("DELETE FROM sujet WHERE id = ?");
        return $stmt->execute([$id]);
    }
    
    public function getCountByCategorie($categorie_id) {
        $stmt = $this->db->prepare("SELECT COUNT(*) as count FROM sujet WHERE categorie_id = ?");
        $stmt->execute([$categorie_id]);
        $result = $stmt->fetch();
    ...
    public function isOwner($sujet_id, $user_id) {
        ...
        try {
            $stmt = $this->db->prepare("SELECT user_id FROM sujet WHERE id = ?");
